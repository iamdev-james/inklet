...
const DOCX_IMAGE_TYPES: Record<string, DocxImageType> = {
  "image/png": "png",
  "image/jpeg": "jpg",
  "image/jpg": "jpg",
  "image/gif": "gif",
...
function imageRun(dataUri: string, alt: string, state: WalkState): InlineChild {
  ...
  return new ImageRun({
    ...
    altText: { title: alt || "Embedded image", description: alt || "Embedded image", name: alt || "image" },
  });
}
...
function collectInline(node: Node, style: InlineStyle, state: WalkState): InlineChild[] {
  ...
  switch (tag) {
    ...
    case "a": {
      ...
      const linkRuns = inner.filter((child): child is TextRun => child instanceof TextRun);
      if (linkRuns.length === 0) return inner;
...
function paragraphFromInline(
  ...
  return [new Paragraph({ children, ...extra })];
}
...
function headingLevelFor(tag: string): (typeof HeadingLevel)[keyof typeof HeadingLevel] {
  switch (tag) {
    case "h1":
    case "h2":
...
function listParagraphs(listEl: Element, level: number, state: WalkState, ordered: boolean, instance: number): BlockChild[] {
  ...
  for (const li of Array.from(listEl.children).filter((child) => child.tagName.toLowerCase() === "li")) {
    ...
    const children = inlineNodes.flatMap((node) => collectInline(node, {}, state));
    ...
    for (const nested of nestedLists) {
      ...
      blocks.push(...listParagraphs(nested, level + 1, state, nestedOrdered, nestedInstance));
    }
  }
  return blocks;
}

function preParagraphs(el: Element): Paragraph[] {
...
function blockquoteParagraphs(el: Element, state: WalkState): BlockChild[] {
  const blocks: BlockChild[] = [];
  for (const node of Array.from(el.childNodes)) {
    if (node.nodeType === 1 && (node as Element).tagName.toLowerCase() === "p") {
      ...
      if (children.length > 0 && node.textContent?.trim()) {
        blocks.push(new Paragraph({ children, ...QUOTE_PROPS }));
      }
    }
  }
  return blocks;
...
function tableFrom(el: Element, state: WalkState): Table | null {
  ...
  for (const tr of Array.from(el.querySelectorAll("tr"))) {
    ...
    for (const cell of Array.from(tr.children)) {
      ...
      const children = content.length > 0
        ? content
...
function blocksFromChildren(parent: Element, state: WalkState): BlockChild[] {
  ...
  for (const node of Array.from(parent.childNodes)) {
    ...
    switch (tag) {
      ...
      case "h5":
      case "h6":
        blocks.push(...paragraphFromInline(el, state, { heading: headingLevelFor(tag) }));
        break;
      case "ul":
        blocks.push(...listParagraphs(el, 0, state, false, 0));
        break;
      case "ol":
        blocks.push(...listParagraphs(el, 0, state, true, state.olInstance++));
        break;
      case "blockquote":
        blocks.push(...blockquoteParagraphs(el, state));
        break;
      case "dl":
...
export const docxConverter: Converter = {
  ...
  async convert(doc) {
    ...
    const dom = new JSDOM(doc.contentHtml, { virtualConsole });
    ...
    const contentBlocks = blocksFromChildren(dom.window.document.body, state);
    ...
    const referenceList = (label: string, refs: typeof doc.references): BlockChild[] =>
      refs.length === 0
        ? []
        : [
            new Paragraph({
    ...
    const document = new Document({
        ...
        config: [
          {
            ...
            levels: [0, 1, 2].map((level) => ({
              level,
              format:
                level === 0 ? LevelFormat.DECIMAL : level === 1 ? LevelFormat.LOWER_LETTER : LevelFormat.LOWER_ROMAN,
              text: `%${level + 1}.`,
      ...
      styles: {
        default: {
          document: { run: { font: SERIF, size: 23, color: INK } },
        },
        paragraphStyles: [
          {
            id: "Title",
      ...
      sections: [
        {
          children: [...header, ...contentBlocks, ...references],
        },
      ],
    });
